            ...
            <div class="category-block">
              <div class="category-label">
                ...
                <div class="category-actions">
                  @if (postForm.get('categoriaId')?.value) {
                    <button type="button" class="action-btn delete-cat-btn" title="Eliminar categoría seleccionada" (click)="deleteCategory(postForm.get('categoriaId')?.value)">🗑️</button>
                  }
                  <button type="button" class="link-btn" (click)="showNewCategory = !showNewCategory">
              ...
              <select class="form-control" formControlName="categoriaId">
                ...
                @for (cat of categories; track cat.id) { <option [value]="cat.id">{{ cat.nombre }}</option> }
              </select>
              @if (showNewCategory) {
                <div class="new-category">
                  <input class="form-control" [value]="newCategoryName" (input)="newCategoryName = $any($event.target).value" placeholder="Nombre de la nueva categoría" />
                  <button type="button" class="btn btn-secondary btn-sm" (click)="createCategory()">Añadir</button>
                </div>
              }
            </div>
...
export class AdminBlogComponent implements OnInit {
  ...
  showNewCategory = false; newCategoryName = '';
  ...
  ngOnInit(): void { this.loadPosts(); this.loadCategories(); this.mediaService.getAllMedia().subscribe({next: media => this.mediaFiles = media}); }
  loadPosts(): void { this.blogService.getAllBlogPosts(true).subscribe({next: posts => this.posts = posts, error: () => this.posts = []}); }
  loadCategories(): void { this.categoryService.list().subscribe({next: categories => this.categories = categories, error: () => this.categories = []}); }
  openCreateForm(): void { this.editingPostId = null; this.selectedCover = null; this.showNewCategory = false; this.newCategoryName = ''; this.postForm.reset({titulo:'',categoriaId:'',portadaId:null,resumen:'',contenidoHtml:'',estado:'BORRADOR'}); this.showForm = true; setTimeout(() => this.editor?.nativeElement.replaceChildren()); }
  editPost(post: BlogPost): void { this.editingPostId = post.id!; this.selectedCover = post.portada ?? null; this.showNewCategory = false; this.newCategoryName = ''; this.postForm.patchValue({titulo:post.titulo,categoriaId:post.categoria.id,portadaId:post.portada?.id ?? null,resumen:post.resumen ?? '',contenidoHtml:post.contenidoHtml,estado:post.estado}); this.showForm = true; setTimeout(() => { if (this.editor) this.editor.nativeElement.innerHTML = post.contenidoHtml; }); }
  closeForm(): void { this.showForm = false; this.editingPostId = null; this.mediaPickerTarget = null; }
  onEditorInput(): void { const html = this.editor?.nativeElement.innerHTML ?? ''; this.postForm.patchValue({contenidoHtml: html.trim() ? html : ''}); }
  ...
  createCategory(): void { const nombre = this.newCategoryName.trim(); if (!nombre) return; this.categoryService.create(nombre).subscribe({ next: (cat) => { this.categories = [...this.categories, cat]; this.postForm.patchValue({ categoriaId: cat.id }); this.newCategoryName = ''; this.showNewCategory = false; }, error: (err) => alert('Error al crear categoría: ' + (err.error?.error ?? 'error desconocido')) }); }
  deleteCategory(id: any): void { if (!id) return; if (confirm('¿Eliminar la categoría seleccionada?')) { this.categoryService.delete(Number(id)).subscribe({ next: () => { this.categories = this.categories.filter(c => c.id !== Number(id)); this.postForm.patchValue({ categoriaId: '' }); }, error: (err) => alert('Error al eliminar categoría: ' + (err.error?.error ?? 'error desconocido')) }); } }
